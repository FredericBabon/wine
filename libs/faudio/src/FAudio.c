...
uint32_t FAudio_Release(FAudio *audio)
{
	...
	if (audio->refcount == 0)
	{
		...
		if (audio->wsolaMergeBuf) {
			audio->pFree(audio->wsolaMergeBuf);
		}
...
uint32_t FAudio_Initialize(
	...
	/* WSOLA: init */
	{
		...
		audio->wsolaHanningSize  = hanning_size;
		...
		if (
			...
			for (i = 0; i < hanning_size; i++) {
				float phase = (float)i / (float)(hanning_size - 1);
				audio->wsolaHannWindow[i] = 0.5f * (1.0f - FAudio_cosf(2.0f * 3.14159265359f * phase));
			}

			FAudio_zero(audio->wsolaMergeBuf, hanning_size * sizeof(float));
		}
